...
exports.insertRandomSitters = function(req, res){
...
exports.insertRandomParents = function(req, res){
...
//var work_table = ["1111111","0111100","0111100","01111100","0111110","0011110","0000110"];
var work_table = ["1111111","0111100","0111100","0011110","0111110","0111110","0001110"];
var brief_table = ["난 잘나가는 부모!!","안녕하세요~~","아이들을 정말 사랑해요","신난다 엄마에요","잘 부탁부탁드립니다","우헤헤헤 신나요","진정한 엄마를 찾으신다면~~"];
var intro_table = ["난 잘나가는 부모!!","안녕하세요~~","아이들을 정말 사랑해요","신난다 엄마에요","잘 부탁부탁드립니다","우헤헤헤 신나요","진정한 엄마를 찾으신다면~~"];
...
//set the user's local credential
		newUser.personal_info.email = "test"+i+"@parent.com";
		newUser.personal_info.passwd = createHash("password");
		newUser.personal_info.name = "테스트"+i;
		newUser.personal_info.birth_date = (10000*(1970+(i%20)))+1122;
		newUser.personal_info.phone = "01043214322";
...
//		동남 : 37.5502042,127.2567293
		...
		newUser.personal_info.lng = lng;
		newUser.personal_info.lat = lat;
		newUser.personal_info.user_type = 0;
//		newUser.personal_info.reg_date = req.body.reg_date;
//		newUser.personal_info.last_login = req.body.last_login;
		newUser.personal_info.push_id = "pushpushpush";
...
//children_info
		...
		newUser.parent_info.gender = 0;
		newUser.parent_info.term_from = 0;
		newUser.parent_info.term_to = 0;
		newUser.parent_info.commute_type = 1;
		newUser.parent_info.distance_limit = 72;
		newUser.parent_info.skill = "";
		newUser.parent_info.work_exp = 0;
		newUser.parent_info.sitter_age = 30;
		newUser.parent_info.childer_info = [ { "name" : "덕소로 ", "is_expect" : 0, "is_care" : 0, "gender" : 0, "birth" : 19801022 } ] ;
//0111110
		newUser.parent_info.mon = work_table[Math.floor(Math.random()*7)];
		newUser.parent_info.tue = work_table[Math.floor(Math.random()*7)];
		newUser.parent_info.wed = work_table[Math.floor(Math.random()*7)];
		newUser.parent_info.thu = work_table[Math.floor(Math.random()*7)];
		newUser.parent_info.fri = work_table[Math.floor(Math.random()*7)];
		newUser.parent_info.sat = work_table[Math.floor(Math.random()*7)];
		newUser.parent_info.sun = work_table[Math.floor(Math.random()*7)];
//0 : 협의
//6000 :  6,000원
//7000 : 7,000원
//8000 : 8,000원 : Default
...
//1500000 : 150만원
//2300000 : 230만원
//		newUser.parent_info.salary = 6000 + Math.floor(Math.random()*9)*1000;
		newUser.parent_info.salary = 1500000 + Math.floor(Math.random()*8)*100000;
		newUser.parent_info.env_pet = 0;
		newUser.parent_info.env_cctv = 0;
		newUser.parent_info.evn_adult = 0;
		newUser.parent_info.religion = 0;
		newUser.parent_info.nat = 0;
		newUser.parent_info.introduction = intro_table[Math.floor(Math.random()*7%7)];
...
exports.removeSitters = function(req, res){
...
exports.removeParents = function(req, res){

	logger.info('Try to find and remove a certain user for a given email - start');

	var query = User.remove({"personal_info.email":{$regex:"parent.com"}}, function(err) {
		if (err)
			res.send(500, err);
                res.json("deleted :(");
	});
